...
def extract_model_name(model_name):
    ...
    if model_name == "CNP" or model_name.find("CNP") >= 0:
        model = "CNP"
    elif model_name == "NP" or ((model_name.find("NP") >= 0) and (not model_name.find("ANP") >= 0)):
        model = "NP"
    elif model_name == "ANP" or model_name == "ANP_variational" or \
            ((model_name.find("ANP") >= 0) and (not model_name.find("log_normal") >= 0) \
                and (not model_name.find("weibull") >= 0)):
        
        if model_name.find("variational") >= 0:
            model = "ANP_variational"
        else:
            model = "ANP"
    elif model_name == "ANP_log_normal" or model_name.find("log_normal") >= 0:
        # model name의 "_" 으로 split을 진행하고 그 중에 앞선 2개만 가져온다. 
        split_str = model_name.split("_")

        if split_str[3] != '0':
            model = "_".join(split_str[:4])
        else:
            model = "_".join(split_str[:5])

        #model = "ANP_log_normal"
    elif model_name == "ANP_weibull" or model_name.find("weibull") >= 0:
        # model name의 "_" 으로 split을 진행하고 그 중에 앞선 2개만 가져온다. 
        split_str = model_name.split("_")
        temp_name = split_str[:3]
        model = '_'.join(temp_name)
...
def set_model(model_name, config, device):
    if model_name == "CNP" or model_name.find("CNP") >= 0:
        ...
        # Attention Modules
        if model_name == "ANP" or model_name == "ANP_variational" or \
            ((model_name.find("ANP") >= 0) and (not model_name.find("log_normal") >= 0) \
                and (not model_name.find("weibull") >= 0)):
...
if __name__ == "__main__":
    ...
    print(a.find('weibull'))

    print(a[:a.find("weibull")])
