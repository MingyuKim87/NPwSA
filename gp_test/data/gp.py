...
class GPData(Dataset):
    def __init__(self, num_tasks, num_points, x_dim=1, y_dim=1, sigma_scale=3., l1_scale=3., n_freq=30, **kargs):
        ...
        # main kernel type
        self._kernel_type = kargs.get('kernel_type', 'rbf')
    ...
    def __set_kernels(self, **kargs):        
        ...
        if self._is_random:
            ...
            sigma_scale = kargs.get("sigma_scale")
            l1_scale = kargs.get("l1_scale")
        else:
            NotImplementedError
        ...
        if self._kernel_type == 'rbf':
            ...
            kernel = GPy.kern.src.stationary.Matern32(input_dim=self._x_dim,\
            variance=sigma_scale,\
            lengthscale=l1_scale
            )

        elif self._kernel_type == 'periodic':
            kernel = GPy.kern.src.periodic.PeriodicExponential(input_dim=self._x_dim,\
            variance=sigma_scale,\
            lengthscale=l1_scale
            )
