...
class meta_1d_regressor_trainer(object):
    def __init__(self, models, criterion, device, data_loader,\
        ...
        num_context = 3, num_extra_target=3, is_average_by_points=False, is_iwae=False, iw_samples=5):
    ...
    def _epochs_test(self, data_loader, is_plot=False):
        ...
        for i, data in enumerate(data_loader):
            # data
            x, y, num_total_point = data
            
            # num_context, num_target (num_total_point - num_context)
            num_context = np.random.randint(low=self.num_context, high=(num_total_point - (self.num_context + self.num_extra_target)))
            #num_extra_target = np.random.randint(low=self.num_extra_target, high=(num_total_datapoints - num_context))
