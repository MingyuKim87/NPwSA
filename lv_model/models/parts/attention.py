...
class Attention(nn.Module):
    ...
    def forward(self, x1, x2, v):
        ...
        if self._att_type == 'uniform':
            ...
            representation = self.laplace_attention(q, k, v, self._scale, self._normalize)
        elif self._att_type == 'dot_prodcut':
            representation = self.dot_product_attention(q, k, v, self._normalize)
        elif self._att_type == 'multihead':
            representation = self.multihead_attention(q, k, v)
        elif self._att_type == 'soft_attention_log_normal':
            representation = self.soft_attention_log_normal(q, k, v, self._scale, self._normalize, \
            ...
            representation = self.soft_attention_weibull(q, k, v, self._scale, self._normalize, \
                self.prior_type, self.eps, self.training,\
                self.k_weibull)
        elif self._att_type == 'soft_multihead_log_normal':
            representation = self.soft_multihead_attention_log_normal(q, k, v, self._scale, self._normalize, \
                self.prior_type, self.eps, self.training, \
                self.sigma_normal_posterior, self.sigma_normal_prior)
        elif self._att_type == 'soft_multihead_weibull':
            representation = self.soft_multihead_attention_weibull(q, k, v, self._scale, self._normalize, \
                self.prior_type, self.eps, self.training,\
                self.k_weibull)
    ...
    def multihead_attention(self, q, k, v):
        ...
        output = self.dot_product_attention(q_prime, k_prime, v_prime, normalize=True)
    ...
    def soft_attention_log_normal(self, q, k, v, scale, normalize,\
        ...
        # add kl loss and attention score
    ...
    def soft_multihead_attention_log_normal(self, q, k, v, scale, normalize,\
        ...
        output = self.soft_attention_log_normal_2(q_prime, k_prime, v_prime, scale=scale, normalize=normalize,\
    ...
    def soft_multihead_attention_weibull(self, q, k, v, scale, normalize,\
        ...
        # soft_attention_log_normal
        output = self.soft_attention_weibull_2(q_prime, k_prime, v_prime, scale=scale, normalize=normalize,\
            prior_type=prior_type, eps=eps, training=training, \
            k_weibull=k_weibull)
