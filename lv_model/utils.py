...
def context_target_split_trainer_LVmodel(batch, **kargs):
    """
        Args:
            batch : attrDict (x, y, x_context, y_context, x_target, y_target)
            is_test : use all or sample
    ...
    x_context = torch.squeeze(batch['xc'], axis=0)
    y_context = torch.squeeze(batch['yc'], axis=0)
    x_target = torch.squeeze(batch['x'], axis=0)
    y_target = torch.squeeze(batch['y'], axis=0) 
