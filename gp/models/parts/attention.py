...
class Attention(nn.Module):
    ...
    def soft_attention_by_dirichlet_vae(self, q, k, v, scale, normalize,\
        prior_type="contextual",eps=1e-10, beta=1.0):
        '''
        ...
        # DEBUG
        debug_matrix = torch.where(torch.isnan(weights), 1, 0)
        indices = debug_matrix.nonzero()
        if indices.sum() > 0:
            print(indices.data)
            ...
            print("-"*10, "weights", "-"*10)
            print(weights.data[index_0, index_1, index_2, :])

            print("-"*10, "sample_gamma", "-"*10)
            print(weights.data[index_0, index_1, index_2, :])

            print("-"*10, "posterior_alpha", "-"*10)
            print(posterior_alpha.data[index_0, index_1, index_2, :])

            print("-"*10, "term_1", "-"*10)
            print(term_1.data[index_0, index_1, index_2, :])

            print("-"*10, "term_2", "-"*10)
            print(term_2.data[index_0, index_1, index_2, :])

            print("-"*10, "values", "-"*10)
            print(v.data[index_0, index_1, :, index_3])
    ...
    def soft_attention_by_dirichlet(self, q, k, v, scale, prior_type="contextual",eps=1e-10):
        '''
            refer to Dirichlet VAE
        '''
        ...
        # Posterior sampling and KL
        dir_post_dist = torch.distributions.dirichlet.Dirichlet(posterior_alpha)
        dir_prior_dist = torch.distributions.dirichlet.Dirichlet(prior_alpha)
        sample_dirichlet = dir_post_dist.rsample()

        # Test
        print(sample_dirichlet.sum(dim=-1))

        # Softmax
        weights = F.softmax(sample_dirichlet, dim=-1)

        # satisfy the simplex constraint    
        # weights = sample_dirichlet / sample_dirichlet.sum(dim=-1, keepdim=True) #[batch_size, # of total_point, # of context_point]

        # compute KLD
        KL = torch.distributions.kl_divergence(dir_post_dist, dir_prior_dist)
        ...
        if indices.sum() > 0:
            ...
            print("-"*10, "sample_dirichlet", "-"*10)
            print(sample_dirichlet.data[index_0, index_1, index_2, :])
    ...
    def soft_multihead_attention_dirichlet(self, q, k, v, scale, normalize,\
        ...
        # soft_attention_log_normal
        # output, weights = self.soft_attention_dirichlet_vae(q_prime, k_prime, v_prime, scale=scale, normalize=normalize,\
            # prior_type=prior_type, eps=eps, beta=beta)

        output, weights = self.soft_attention_by_dirichlet(q_prime, k_prime, v_prime, scale=scale, prior_type=prior_type, eps=eps)
...
if __name__ == "__main__":
    a = torch.randn([100, 8, 40, 5])

    dir_dist = torch.distributions.dirichlet.Dirichlet(a)
    normal_dist = torch.distributions.normal.Normal(loc=a, scale=1)
    b = dir_dist.rsample()
    c = b.sum()

    d = normal_dist.rsample()
    e = d.sum()
